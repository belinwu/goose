...
mod tests {
    ...
    fn test_home_directory_conversion() {
        ...
        assert_eq!(
            shorten_path("/Users/testuser/.config/test.txt"),
            "~/.config/test.txt"
        );
    ...
    fn test_long_path_shortening() {
        assert_eq!(
            shorten_path("/very/long/path/with/many/components/file.txt"),
            "/v/l/p/w/m/components/file.txt"
        );

        // Test with home directory in long path
        env::set_var("HOME", "/Users/testuser");
        assert_eq!(
            shorten_path("/Users/testuser/Development/project/src/lib/utils/test.rs"),
            "~/D/p/s/l/utils/test.rs"
        );
    }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            shorten_path("/path/with/@#$/special/chars.txt"),
            "/p/w/@/special/chars.txt"
        );

        // Path with empty components
        assert_eq!(shorten_path("//usr//local//bin//file"), "//u/l/bin/file");

        // Path with unicode characters
        assert_eq!(
            shorten_path("/usr/🦆/path/to/file.txt"),
            "/u/🦆/to/file.txt"
        );

        // Path with spaces
        assert_eq!(
            shorten_path("/path/with spaces/more spaces/file.txt"),
            "/p/with spaces/more spaces/file.txt"
        );
    }
}
